...
const CommentList = ({ refList, comments, removeComment, set_state, setStateUpComp, isCommentDelete, replyComment, globalSpin, dataListChats, filterChatRooms, dataFilteredChats, scrollToBottom,
  ...
  const currentDialog = dataListChats.find(it => it.id === chatId);
  ...
  return <ul
    ...
    >
    { globalSpin ? null : comments.length > 0 ? comments[0].isfirst ? <div style={styles.createInfo}><span style={{fontWeight: 'bolder'}}>{currentDialog.creator.login}</span> created{createdTime}</div> : null
                          : <div style={styles.createInfo}><span style={{fontWeight: 'bolder'}}>{currentDialog.creator.login}</span> created {createdTime}</div>}
    { globalSpin ? <Spin style={{margin: 'auto', fontSize: 30}} tip='Загрузка...' size='large' /> : comments.map((item, ind) => {
        ...
        <li
          ...
          style={styles.comments__li}>
            ...
            <ContextMenuTrigger id={`${item.id}`}>
              ...
              >
                <div style={styles.comments__itemContainer}>
                  ...
                  { item.forwarded_from !== null ? <span style={{fontWeight: 'bolder'}}>Forwarded from <span style={{backgroundColor: '#c8c8c8', color: '#4586c2', fontWeight: 'bolder', padding: '0 5px', borderRadius: 10}}>{item.forwarded_from_user}</span></span> : null}
                ...
                {item.reply_message ? <Link to={`chat#${item.reply_message.id}`} scroll={el => {
                                        ...
                                        <div onClick={() => {
                                          ...
                                          <div>{item.reply_message.message_text.length > 100 ? item.reply_message.message_text.split('\n').map((it, key) => {
                                                                                                                                          ...
                                                                                                                                          }) : item.reply_message.message_text}</div>
                                          {item.reply_message.files.length > 0 ? <div style={{backgroundColor: 'aliceblue', padding: '0 5px', borderRadius: 10}}>
                                                                                    {item.reply_message.files[0].filename}
                                                                                 </div> : null}
                                          {item.reply_message.images.length > 0 ? <div>
                                                                                    <div><span style={{backgroundColor: 'aliceblue', margin: '2px 0', padding: '0 5px', borderRadius: 10}}>Image:</span></div>
                                                                                    <img src={api._url + item.reply_message.images[0].uri} height={40} />
                                                                                  </div> : null}
                                        </div>
                                      </Link> : null}
                {item.images.length > 0 ? item.id === isCommentDelete ? <Spin style={{marginLeft: 20, marginTop: 10}}  />
                                                                    : item.images.map(it => <div key={it.uri} style={{borderRadius: 10, border: 'black', height: 200, cursor: 'pointer', marginTop: 5}} >
                  ...
                  >{item.files.map((file) => (
                    <li key={file.uri} className='getone__filelist-item'>
                      ...
                      <div style={{paddingLeft: 40, paddingRight: 20}}>
                        <Tooltip title='Скачать' placement='topLeft'>
            ...
            <ContextMenu
              ...
              onHide={() => enableScroll()}
            >
              <MenuItem onClick={() => replyComment(item)}>Answer</MenuItem>
              <MenuItem onClick={() => set_state({isModalForwardOpen: true, commentForwardID: item.id})}>Forward message</MenuItem>
              {item.ismine ?
                            (item.files.length > 0 || item.images.length > 0) ? null
                            : item.forwarded_from ? null
                                                  : diffHours > 24 ?
                                                                    null
                                                                    : <MenuItem
                                                                        onClick={() => {
    ...
    >
      <div style={{fontSize: 30}}>Are you sure?</div>
    </Modal>
    <Modal
      centered
      footer={null}
    ...
    >
      ...
      <List
        ...
        bordered
        dataSource={valueSearch === '' ? dataListChats : dataFilteredChats}
        renderItem={it => (
...
const enhance = compose(
  connect(
    state => ({
      ...
      is_first_id: state.user.is_first_id
    }),
    dispatch => ({
  ...
  withHandlers({
    ...
    filterChatRooms: ({dataListChats, set_state}) => value => {
      ...
      const filteredChats = dataListChats.filter(item => item.title.toLocaleLowerCase().includes(_value));
  ...
  lifecycle({
    ...
    componentDidUpdate(prevProps){
      const { refList, comments, refLastEl, setStateUpComp, changedId, is_first_id, set_first_id, isSubmit, set_state, isScrolled, elemTopScroll } = this.props;
