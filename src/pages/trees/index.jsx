...
const Trees = ({
  history, location, getData,
  view, ready, openedKeys, menu,
  handlerOpenChange, handlerSelectMenu,
  values, params
}) => {
  function menuRender(arr, fnMenu) {
    
    return arr.map((menuItem, menuIndex) => {
      return menuItem.children ? <SubMenu
          key={menuItem.key}
          data={menuItem}
          title={
            <span>
        ...
        >{fnMenu(menuItem.children, fnMenu)}</SubMenu>: <Menu.Item key={menuItem.key} data={menuItem}>
              <span>
                <Icon type={menuItem.icon || ''} />
                <span>{menuItem.label || '--'}</span>
              </span>
  ...
  return [
    <MyHeader key='s1' history={history} title={''}>
      { !(_.isEmpty(values)) ? <b style={{marginLeft: '5rem', color: '#001529', fontSize: '1.1em'}}>{values.title.toUpperCase()}</b> : null }
    </MyHeader>,
    <Row
      key='s2'
      gutter={8}
        ...
        <Layout.Content>
          <Spin tip='Loading...' spinning={!ready}>
            { 
              ready ? view.treeviewtype === 1 ?
                (() => {
                  switch(view.viewtype) {
                    case 'table':
                      return <Row style={{ margin: '0 10px' }}>
              ...
              : view.treeviewtype === 2 ? <Composition trees = {true} compo = {true} path = {view.path}  history = {history} location={location} /> : null
              : null
            }
          </Spin>
        </Layout.Content>
